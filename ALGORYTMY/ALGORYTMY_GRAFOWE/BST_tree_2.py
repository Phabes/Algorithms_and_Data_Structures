# Algorytm przepina węzły, nawet przy usuwaniu elementów z dziećmi po obu stronach
...
def delete(root, val):
  ...
  # Element ma oba poddrzewa
    ...
    x = findNext(node)
    # Aktualizujemy rodziców dzieci
    node.left.parent = x
    node.right.parent = x
    # Aktualizujemy znalezionego następnika
    x.left = node.left
    if node.right != x:
    ...
    if node == root:
      return x
    # Aktualizujemy rodzica usuwanego elementu
    if node.parent.left == node:
      node.parent.left = x
    else:
      node.parent.right = x
